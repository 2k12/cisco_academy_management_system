...
// ! ↓ estas comente
import RegisterCostModal from "../Cost/RegisterCostModal";
// import ReportsModalityModal from "../Reports/ReportsModalityModal";

function CostPage() {

    const { totalPages, currentPage, setCurrentPage,costs,deleteCost,getCosts} = useCosts();
    ...
    useEffect(() => {
        getCosts({ search: searchTerm, page: currentPage });
    ...
    const handleDelete = (detailValueId) => {
        deleteCost(detailValueId); // Llama a la función de eliminación
        }
    // };
    ...
    return (
        <div className="p-6 bg-zinc-750 min-h-screen text-white overflow-hidden">
            <div className="flex justify-between mb-6">
                <input
                    type="text"
                    placeholder="Buscar Costos de Cursos"
                    value={searchTerm}
            ...
            <div className="scroll-hidden dark:bg-gray-800 relative overflow-x-auto shadow-md sm:rounded-lg max-h-[600px] min-h-[600px]">
                <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                    ...
                    <tbody>
                        {costs.map((cost) => (
                            <tr key={cost.cost_id} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-700">
                                <th scope="row" className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                    {cost.cost_id}
                                </th>
                                <td className="px-6 py-4">{cost.amount}</td>
                                <td className="px-6 py-4">{cost.description}</td>
                                <td className="px-6 py-4">{cost.Details[0].Courses[0].course_name}</td>
                                <td className="px-6 py-4 flex space-x-4">
                                    <button className="text-blue-500 hover:text-blue-700">
                                        <FontAwesomeIcon icon={faEye} />
                                    </button>
                                    <button className="text-yellow-500 hover:text-yellow-700" onClick={() => handleEdit(cost)}>
                                        <FontAwesomeIcon icon={faEdit} />
                                    </button>
                                    <button className="text-red-500 hover:text-red-700" onClick={() => handleDelete(cost.cost_id)}>
            ...
            {/* ! estas comente igual */}
            <RegisterCostModal isOpen={isRegisterModalOpen} onClose={() => setIsRegisterModalOpen(false)} cost={selectedCost} />
